...
while command != "course start":
    ...
    if action == "Add":
        ...
        if lesson_title in lessons and lesson_title2 in lessons:
            i1 = lessons.index(lesson_title)
            i2 = lessons.index(lesson_title2)
            lessons[i1], lessons[i2] = lessons[i2], lessons[i1]

            exercise1 = f"{lesson_title}-Exercise"
            if exercise1 in lessons:
                lessons.remove(exercise1)
                new_index1 = lessons.index(lesson_title)
                lessons.insert(new_index1 + 1, exercise1)

            exercise2 = f"{lesson_title2}-Exercise"
            if exercise2 in lessons:
                lessons.remove(exercise2)
                new_index2 = lessons.index(lesson_title2)
                lessons.insert(new_index2 + 1, exercise2)

    elif action == "Exercise":
        if lesson_title in lessons:
            if f"{lesson_title}-Exercise" not in lessons:
                index_lesson = lessons.index(lesson_title)
                lessons.insert(index_lesson + 1, f"{lesson_title}-Exercise")
        else:
            lessons.append(lesson_title)
